...
set onecmd="%npackd_cl%\npackdcl.exe" path --package=com.selenic.mercurial.Mercurial64 --versions=[1.8,2)
for /f "usebackq delims=" %%x in (`%%onecmd%%`) do set mercurial=%%x

set onecmd="%npackd_cl%\npackdcl.exe" path --package=net.sourceforge.gnuwin32.CoreUtils --versions=[5.3,5.3]
for /f "usebackq delims=" %%x in (`%%onecmd%%`) do set coreutils=%%x
