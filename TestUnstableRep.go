...
func exec2(command string) (exitCode int, output []string) {
    ...
    // only on Windows cmd.SysProcAttr.CmdLine = command
...
func downloadRepos(settings *Settings) {
    ...
    exec("\"" + settings.git + "\" checkout master");

    exec("\"" + settings.curl + "\" -f -o repository\\RepUnstable.xml " +
	 "https://www.npackd.org/rep/xml?tag=unstable");
    exec("\"" + settings.curl + "\" -f -o repository\\Rep.xml " +
	 "https://www.npackd.org/rep/xml?tag=stable");
    exec("\"" + settings.curl + "\" -f -o repository\\Rep64.xml " +
	 "https://www.npackd.org/rep/xml?tag=stable64");
    exec("\"" + settings.curl + "\" -f -o repository\\Libs.xml " +
	 "https://www.npackd.org/rep/xml?tag=libs");

    exec("\"" + settings.git + "\" config user.email \"tim.lebedkov@gmail.com\"");
    exec("\"" + settings.git + "\" config user.name \"tim-lebedkov\"");
    exec("\"" + settings.git + "\" commit -a -m \"Automatic data transfer from https://www.npackd.org\"");
    exec("\"" + settings.git + "\" push https://tim-lebedkov:" + settings.githubToken +
